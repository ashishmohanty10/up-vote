...
io.on("connection", async (socket) => {
  ...
  socket.on("join-room", async (room: string) => {
    ...
    const subscribedRoutes = await redis.smembers("subscribed-rooms");
    ...
    if (!subscribedRoutes.includes(room)) {
      ...
      if (remainingConnections <= 0) {
        ...
        subRedis.unsubscribe(room, async (err) => {
          if (err) {
            ...
            console.log("Unscribed from room", room);
          }
        });
      }
    });
...
server.listen(PORT, () => {
  console.log(`Server is listening in post ${PORT}`);
});
